...
def run() -> None:
    ...
    log.info("Старт: %d кошельков, сети=%s, порог=%d, опрос каждые %ds",
             len(watchlist), list(clients), min_wallets, poll_s)
        ...
        try:
            ...
            for a in alerts:
                embed = build_embed(name=a.name, chain=a.chain,
                                    contract=a.contract, wallets=a.wallets)
                try:
                    post_alert(webhook, embed, role_id=role_id)
                    log.info("Алерт: %d кош. минтят %s (%s)",
                             len(a.wallets), a.name, a.chain)
