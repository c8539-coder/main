...
**Команда/трежери исключается — по батч-минту в одном блоке.** Кошелёк
помечается `is_team`, если заминтил `>= TEAM_BATCH_MIN` (по умолчанию 50) токенов
**в одном блоке** (одной пачкой). Это отличает пре-минт аллокацию команды от кита
открытого минта: кит/бот набирает по чуть-чуть в течение тысяч блоков (max в
одном блоке маленький), а команда минтит всю пачку разом. Раньше отсекали по
суммарному числу минтов — это ошибочно прятало обычных крупных минтеров в
открытых минтах. Такой кошелёк не получает early-кредита и убирается из
лидерборда (в CSV остаётся `is_team=1` и `mint_batch`). Вернуть — `--keep-team`.
