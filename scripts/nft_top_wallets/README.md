...
**Команда/трежери исключается.** Кошелёк, заминтивший `>= TEAM_MINT_MIN`
(по умолчанию 10) токенов, помечается `is_team` (батч-минт аллокации) — он
**не получает early-кредита** и **убирается из лидерборда** (в CSV остаётся с
флагом `is_team=1`). Вернуть в вывод — флагом `--keep-team`.
